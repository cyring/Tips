#define AutoDate "@(#) Call Generator - 4.0.1 (C) Bouygues Telecom "__DATE__
...
SOCKETDESC	listenSocket = SOCKETNULL, UdpSocket = SOCKETNULL,
			TcpSocket[CLIENTS] = {SOCKETNULL,SOCKETNULL,SOCKETNULL,SOCKETNULL,SOCKETNULL};
SOCKETSTRUCT	remoteAddr[CLIENTS] = { {0}, {0}, {0}, {0}, {0} };
...
int     SocketSendTo(SOCKETDESC socketdesc, SOCKETSTRUCT *To, char *data)
{
#ifndef	WIN32
		return( sendto( socketdesc, data, SOCKETSIZE, 0, To, sizeof(SOCKETSTRUCT)) );
#else
		fd_set	sockSet;
		struct	timeval timeout = {3, 0};	// 3 sec + 0 µsec
		int		ret = 0;
...
void	strace(int i)
{
	...
	for( client = 0; client < CLIENTS; client++ )		
	    if( TcpSocket[client] != SOCKETNULL ) {
#ifndef WIN32
		  if((len = SocketSendTo( UdpSocket, &remoteAddr[client], buffer )) == -1 )
#else
		  if((len = SocketSendTo( UdpSocket, &remoteAddr[client], buffer )) == SOCKET_ERROR )
#endif
		    fstderr("strace() : send to client(%d) socket(%d) failed %d", client, UdpSocket, WSAGetLastError ());
	}
}
...
void	Command(int socClient, char *cmd )
{
	     if( !strncmp( cmd, "user", 4 ))
			...
			fstderr( "Welcome User <%s>", userName[socClient]);
	     	}
		...
		for( client = 0; client < CLIENTS; client++ )
			...
			len = SocketSend( TcpSocket[socClient], buffer );
			}
		}
...
int	sendConfig(int client)
{
	...
	len = SocketSend( TcpSocket[client], buffer );
...
int	sendConnections(int client, int counters)
{
	...
	len = SocketSendTo( UdpSocket, &remoteAddr[client], buffer );
...
void	serviceThread(long client)
{
	...
	while( !bye )
		{
#ifndef WIN32
		if( (len = SocketRead(TcpSocket[client], buffer, SOCKETSIZE)) == -1) {
#else
		if( ((len = SocketRead(TcpSocket[client], buffer, SOCKETSIZE)) == SOCKET_ERROR) || (len == 0) ) {
#endif
			fstderr("serviceThread(%d) : Can not read socket <%d>. Thread exiting.",client, TcpSocket[client]);
			shutdown( TcpSocket[client], 2 );
			SocketClose( TcpSocket[client] );
	  		bye = 1;
		}
		else
			if(len > 0 ) {
	...
	fstderr("Goodbye User <%s>", userName[client]);
	TcpSocket[client] = SOCKETNULL;
	userName[client][0] = '\0';
}
...
#endif
{
	...
	if( (UdpSocket = socket(AF_INET, SOCK_DGRAM, 0)) == -1) {
		fstderr("UDP socket() failed.");
		bye = 1;
	}
#else
	if( (UdpSocket = socket(AF_INET, SOCK_DGRAM, 0)) == INVALID_SOCKET) {
		fstderr("UDP socket() failed: %ld", WSAGetLastError ());
		bye = 1;
	}
#endif
	...
	while( !bye )
	  ...
	  if( client < CLIENTS ) {
		int	addrSize = sizeof(SOCKETSTRUCT);

#ifndef WIN32
	    if( -1 == (TcpSocket[client] = accept(listenSocket, remoteAddr[client], &addrSize))) {
#else
	    if( INVALID_SOCKET == (TcpSocket[client] = accept(listenSocket, (SOCKADDR *) &remoteAddr[client], &addrSize))) {
#endif
		  ...
		  Sleep(1000);
			...
			remoteAddr[client].sin_port = htons(port);
#ifndef WIN32
...
#else
			serviceTID = _beginthread( serviceThread, 0, (void *) client);
#endif
			}
	    }
	  else
