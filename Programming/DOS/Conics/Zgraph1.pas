...
var
          depth,xshift,yshift,zshift: integer;

procedure Trigo(xa,ya,za: integer);
procedure Zmove(xe,ye,ze: real);
procedure Zplot(xe,ye,ze: real; pixel: word);
procedure Zline(xe1,ye1,ze1,xe2,ye2,ze2: real; pixel: word);
...
begin
     ...
     MoveTo(trunc(xproj),trunc(yproj));
end;
...
begin
     ...
     yproj:= yrot/q + GetMaxY / 2;
     PutPixel(trunc(xproj),trunc(yproj),pixel);
end;
...
begin
     ...
     SetColor(pixel);
     Line(trunc(xproj1),trunc(yproj1),trunc(xproj2),trunc(yproj2));
end;

procedure ZFillPoly;              { trace un polygone … quatre c“t‚s. }
var       q,xrot,yrot,zrot: real; proj: array[1..4] of PointType; loop: byte;
begin
     for loop:=1 to 4 do
